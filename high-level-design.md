...
│  docs/ja/context_template.md  入力テンプレート（追跡対象）       │
└──────────────────────────────┬──────────────────────────────────┘
...
> テンプレートは `docs/ja/context_template.md` を参照。
...
動機ベースのカテゴリ（国家支援 / ランサムウェア / ハクティビスト / サイバー犯罪 / インサイダー）を軸とし、業種・地域でフィルタリングする。

**カテゴリ構成:**
- `actor_categories.state_sponsored`: China（Salt Typhoon・Volt Typhoon 等サブグループ含む）/ Russia（Sandworm・Turla・TEMP.Veles）/ North Korea（BlueNoroff・TraderTraitor）/ Iran / India
- `actor_categories.ransomware`: LockBit, RansomHub, Cl0p, Akira, Dark Angels 等 + サブグループ特性
- `actor_categories.hacktivist`: KillNet, NoName057(16)
- `actor_categories.cybercriminal`: FIN7, Scattered Spider, TA505（BEC/詐欺/初期アクセスブローカー）
- `actor_categories.insider_threat`: データ窃取・妨害行為
- `industry_threat_map`: manufacturing / finance / energy / healthcare / defense / technology / logistics / government / education / pharmaceutical / telecom / retail / automotive / aerospace
- `geography_threat_map`: Japan / Southeast Asia / South Korea / Taiwan / Europe / United Kingdom / Germany / USA / Australia / Canada / Middle East / India
- `business_trigger_map`: m_and_a / ot_connectivity / cloud_migration / ipo_or_listing / supply_chain_expansion / regulatory_change / digital_transformation
- `supply_chain_threat_map`（新規）: erp_vendor / it_managed_service_provider / software_supplier / hardware_supplier / cloud_provider / financial_messaging
...
│   │   └── schema.py          # BusinessContext / Organization / CriticalAsset 等の Pydantic モデル
│   ├── analysis/
│   │   ├── element_extractor.py  # Step 1: 要素抽出（critical_assets 含む）
│   │   ├── asset_mapper.py       # Step 2: 資産タグマッピング（critical_assets 対応）
│   │   ├── threat_mapper.py      # Step 3: 脅威マッピング（辞書+LLM補完）
...
│       └── prompts/
│           ├── context_structuring.md   # context.md → BusinessContext JSON 変換プロンプト
│           ├── pir_generation.md        # PIR 文章生成プロンプト
│           └── threat_tag_completion.md # 脅威タグ補完プロンプト
├── cmd/
│   ├── generate_pir.py        # メイン CLI（デフォルト: input/context.md → output/）
│   ├── validate_pir.py        # PIR JSON スキーマ検証
│   └── generate_schemas.py    # Pydantic から JSONSchema を自動生成
├── schema/
│   ├── business_context.schema.json   # JSONSchema for input validation
│   ├── threat_taxonomy.json           # 業種×地域×動機 → 脅威タグ辞書
│   ├── asset_tags.json                # 資産種別 → SAGE タグ辞書（ドメインコントローラー等追加）
...
└── docs/
    ...
    └── ja/
        ├── sage_integration.md
        └── context_template.md    # context.md 記載テンプレート（追跡対象）
```
...
# 中間ファイル(business_context.json)も保存してアナリストがレビューできるようにする
uv run python cmd/generate_pir.py \
