...
`google-cloud-aiplatform` SDK（`vertexai` パッケージ）経由で Gemini モデルを呼び出す。
...
# タスク種別ごとにモデルを切り替える
def get_model(task: str) -> GenerativeModel:
    model_map = {
    ...
    return GenerativeModel(model_map[task])
```
...
├── pyproject.toml             # uv + ruff（pydantic, vertexai を依存に追加）
...
│   └── llm/
│       ├── client.py             # Vertex AI Gemini クライアント（vertexai SDK）
...
| `google-cloud-aiplatform>=1.60` | Vertex AI Gemini 呼び出し（`vertexai` サブパッケージ含む） |
