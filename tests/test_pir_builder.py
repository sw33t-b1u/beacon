...
# ---------------------------------------------------------------------------
# Phase 3 — schema_version field tests (BEACON 0.17.0)
# ---------------------------------------------------------------------------


class TestSchemaVersion:
    """PIROutputDocument.schema_version is always present with default '0.17.0'."""
    ...
    def test_schema_version_default_value(self):
        doc = self._make_doc()
        assert doc.schema_version == "0.17.0"
        dumped = doc.model_dump()
        assert dumped["schema_version"] == "0.17.0"
    ...
    def test_schema_version_not_overrideable_accident(self):
        doc = PIROutputDocument(schema_version="0.17.0", pirs=[])
        assert doc.schema_version == "0.17.0"
