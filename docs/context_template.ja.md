...
- **account_type**: STIX 2.1 §6.4 ``account-type-ov`` のみ。空欄 (該当なし) または `unix` / `windows-local` / `windows-domain` / `ldap` / `tacacs` / `radius` / `nis` / `openid` / `facebook` / `skype` / `twitter` / `kavi`。サービス系は `account_type` 空欄 + `is_service_account: true` で表現する
