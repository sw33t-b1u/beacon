...
## Output: identity_assets.json (Initiative A + Initiative C Phase 2)
...
| `is_high_value_impersonation_target` | bool | **Initiative C Phase 2 (BEACON 0.13.0+)** — when `true`, SAGE 0.9.0 applies `effective_priority` multiplier=1.5 unconditionally on `ImpersonatesIdentity` (vs the `HIGH_VALUE_IMPERSONATION_ROLES` 15-entry role-tag fallback). TRACE 1.6.0 PIR L2 gate adds +0.2 relevance boost when the crawled document mentions a flagged identity by name. |
| `impersonation_risk_factors` | list[string] | **Initiative C Phase 2** — free-form tags (e.g. `["public-facing-brand", "executive", "trusted-supplier"]`). Surfaced on analyst dashboards; **not** used in the `effective_priority` formula. |
...
## Output: user_accounts.json (Initiative B)
...
| `account_type` | enum | STIX 2.1 §6.4 `account-type-ov`: empty string / `unix` / `windows-local` / `windows-domain` / `ldap` / `tacacs` / `radius` / `nis` / `openid` / `facebook` / `skype` / `twitter` / `kavi`. Use empty string + `is_service_account=true` for service accounts (BEACON 0.12.1 dropped non-spec values). |
...
Any active business trigger escalates `tactical` → `operational` regardless of score (NIST SP 800-37 R2 event-driven trigger framework). Triggers are detected from BusinessContext structural fields and corroborated by external citations — see [`docs/triggers.md`](triggers.md) for the ten-trigger contract.
